...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print
        "usage:python run_jobs.py case_id user";
        exit();
    ...
    if not os.path.exists(local_dataset_folder):
        print
        '%s folder do not exist, then create it.' % local_dataset_folder;
    ...
    if not os.path.exists(local_image_folder):
        print
        '%s folder do not exist, then create it.' % local_image_folder;
    ...
    with open(config_json_file) as json_data:
        ...
        db_name2 = d['db_name2'];
        print
        patch_size, db_host, db_port, db_name1, db_name2;
    ...
    def saveFeatures2MongoDB(case_id, image_width, image_height, user, title_index, patch_min_x_pixel,
                             ...
                             Flatness_segment_mean, Flatness_segment_std, Perimeter_segment_mean, Perimeter_segment_std,
    ...
    ######################################################################
    def process_one_patch(case_id, user, tile_index, patch_polygon_area, patch_polygon_area2, image_width, image_height,
        ...
        try:
            ...
            print
            'Handling run-time error:', detail
            exit();
            ...
            for item in nuclues_item_list:
                ...
                if (special_case2 == "within" and computer_polygon.within(patch_polygon)):  # within/within
                    ...
                    if polygon_intersect.geom_type == 'MultiPolygon':
                        ...
                        print
                        "patch indexes %d Shape is not a polygon!!!" % tile_index;
                        print
                        polygon_intersect.geom_type;
                        print
                        polygon_intersect;
        ...
        if (special_case1 == "both" and tumorFlag == "non_tumor"):
            ...
            "\n";
            saveFeatures2MongoDB(case_id, image_width, image_height, user, title_index, patch_min_x_pixel,
                                 patch_min_y_pixel, patch_size, patch_polygon_area2, tumorFlag, nucleus_area,
                                 percent_nuclear_material, grayscale_patch_mean, grayscale_patch_std,
                                 Hematoxylin_patch_mean, Hematoxylin_patch_std, segment_mean_grayscale_intensity,
                                 segment_std_grayscale_intensity, segment_mean_hematoxylin_intensity,
                                 segment_std_hematoxylin_intensity, Flatness_segment_mean, Flatness_segment_std,
    ...
    for item in image_list:
        ...
        if (len(prefix_list)) < 1:
            print
            "can NOT find prefix of this image!"
            exit();

        comp_execution_id = getCompositeDatasetExecutionID(case_id);
        if (comp_execution_id == ""):
            print
            "Composite dataset for this image is NOT available.";
            continue;
        print
        "comp_execution_id is:" + str(comp_execution_id);
        ...
        if (len(humanMarkupList_tumor) == 0 and humanMarkupList_non_tumor == 0):
            print
            "No tumor or non tumor regions has been marked in this image by user %s." % user;
            continue;
        ...
        if not os.path.exists(local_img_folder):
            print
            '%s folder do not exist, then create it.' % local_img_folder;
        ...
        if not os.path.isfile(image_file):
            print
            "image svs file is not available, then download it to local folder.";
        ...
        for prefix in prefix_list:
            ...
            if not os.path.exists(detail_local_folder):
                print
                '%s folder do not exist, then create it.' % detail_local_folder;
                os.makedirs(detail_local_folder);
            if os.path.isdir(detail_local_folder) and len(os.listdir(detail_local_folder)) > 0:
                print
                " all csv and json files of this image have been copied from data node.";
            else:
            ...
            for index, tile_min_point in enumerate(unique_tile_min_point_list):
                ...
                for i in range(0, i_range):
                    for j in range(0, j_range):
                        ...
                        if (
                                patchHumanMarkupRelation_tumor == "disjoin" and patchHumanMarkupRelation_nontumor == "disjoin"):
                            continue;
                        ...
                        executor.submit(process_one_patch, case_id, user, index, patch_polygon_area1,
                                        patch_polygon_area2, image_width, image_height, patch_polygon_0,
                                        patchHumanMarkupRelation_tumor, patchHumanMarkupRelation_nontumor,
                                        patch_humanmarkup_intersect_polygon_tumor,
                                        patch_humanmarkup_intersect_polygon_nontumor, nuclues_item_list);
        img.close();
    exit();
