...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print "Usage:\npython run_jobs.py case_id user"
        exit(1)
    ...
    # print " --- read config_cluster.json file ---"
    ...
    # TODO: CHANGE NAME OUTPUT DATABASE
    # collection_saved = db2.patch_level_features
    collection_saved = db2.feature_test_td
    ...
    try:
        ...
        print('findPrefixList: ', e)
        exit(1)

    try:
        def read_csv_data_file(file_path, data_file):
            # print('read_csv_data_file')
            obj_list = []
            ...
            with open(csv_file, 'rb') as csv_read:
                ...
                for row in reader:
                    ...
                    for i in range(0, len(split_str) - 1, 2):
                        point = [float(split_str[i]) / float(image_width),
    ...
    try:
        ...
        print('findUniqueTileList: ', e)
        exit(1)

    try:
        def getCompositeDatasetExecutionID(case_id):
            # print('getCompositeDatasetExecutionID')
            execution_id = ""
        ...
        print('getCompositeDatasetExecutionID: ', e)
        exit(1)

    try:
        def getPolygonCountFromDB(case_id, comp_execution_id, polygon0):
            # print('getPolygonCountFromDB')
            ...
            for nucleus_polygon in objects2.find({"provenance.image.case_id": case_id,
                ...
                nuclear_polygon = nucleus_polygon["geometry"]["coordinates"][0]
                # exclude those nuclear objects, which are out of patch boundary
        ...
        print('getPolygonCountFromDB: ', e)
        exit(1)

    try:
        def findTumor_NonTumorRegions(case_id, user):
            # print('findTumor_NonTumorRegions')
        ...
        print('findTumor_NonTumorRegions: ', e)
        exit(1)
    ...
    try:
        ...
        print('getMatrixValue: ', e)
        exit(1)

    try:
        def saveFeatures2MongoDB(case_id, image_width, image_height, user, title_index, patch_min_x_pixel,
                                 patch_min_y_pixel, patch_size, patch_polygon_area, tumorFlag, nucleus_area,
                                 percent_nuclear_material, grayscale_patch_mean, grayscale_patch_std,
                                 Hematoxylin_patch_mean, Hematoxylin_patch_std, grayscale_segment_mean,
                                 grayscale_segment_std, Hematoxylin_segment_mean, Hematoxylin_segment_std,
                                 Flatness_segment_mean, Flatness_segment_std, Perimeter_segment_mean,
    ...
    try:
        def process_one_patch(case_id, user, tile_index, patch_polygon_area, image_width, image_height,
                              patch_polygon_original, patchHumanMarkupRelation_tumor, patchHumanMarkupRelation_nontumor,
                              patch_humanmarkup_intersect_polygon_tumor, patch_humanmarkup_intersect_polygon_nontumor,
                              nucleus_item_list):
            # print('process_one_patch')
            ...
            try:
                ...
                print('Error reading region: ', patch_min_x_pixel, patch_min_y_pixel)
                print(e)
                exit(1)
            ...
            print "\nspecial_case1: ", special_case1
            print "tumorFlag: ", tumorFlag
            print "patchHumanMarkupRelation: ", patchHumanMarkupRelation
            # print "patch_humanmarkup_intersect_polygon: ", patch_humanmarkup_intersect_polygon, "\n"

            record_count = len(nucleus_item_list)
            print "record_count: ", record_count
            print "\n"

            if record_count > 0 and special_case1 != "both":
                print("579 record_count > 0 and special_case1 != \"both\"")
                for item in nucleus_item_list:
                    ...
                    if special_case2 == "within" and computer_polygon.within(patch_polygon):  # within/within
                        print("special_case2 == \"within\" and computer_polygon.within(patch_polygon)")
                        ...
                        has_value, one_polygon_mask = getMatrixValue(computer_polygon_points, patch_min_x_pixel,
                                                                     patch_min_y_pixel)
                        if has_value:
                        ...
                        print("special_case2 == \"within\" and computer_polygon.intersects(patch_polygon)")
                    ...
                    elif special_case2 == "intersects" and computer_polygon.within(
                            patch_polygon):  # intersects/within
                        print("671 special_case2 == \"intersects\" and computer_polygon.within(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
                        ...
                        print("special_case2 == \"intersects\" and computer_polygon.intersects(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
            ...
            if (special_case1 != "both"):
                ...
                print "\n"
                saveFeatures2MongoDB(case_id, image_width, image_height, user, tile_index, patch_min_x_pixel,
                                     patch_min_y_pixel, patch_size, patch_polygon_area, tumorFlag, nucleus_area,
                                     percent_nuclear_material, grayscale_patch_mean, grayscale_patch_std,
                                     Hematoxylin_patch_mean, Hematoxylin_patch_std, segment_mean_grayscale_intensity,
                                     segment_std_grayscale_intensity, segment_mean_hematoxylin_intensity,
                                     segment_std_hematoxylin_intensity, Flatness_segment_mean, Flatness_segment_std,
                                     Perimeter_segment_mean, Perimeter_segment_std, Circularity_segment_mean,
                                     Circularity_segment_std, r_GradientMean_segment_mean, r_GradientMean_segment_std,
                                     b_GradientMean_segment_mean, b_GradientMean_segment_std,
                                     r_cytoIntensityMean_segment_mean, r_cytoIntensityMean_segment_std,
                                     b_cytoIntensityMean_segment_mean, b_cytoIntensityMean_segment_std)
            ...
            tumorFlag = "tumor"
            if (record_count > 0 and special_case1 == "both"):
                print("853 record_count > 0 and special_case1 == \"both\"")
                ...
                for item in nucleus_item_list:
                    ...
                    if (special_case2 == "within" and computer_polygon.within(patch_polygon)):  # within/within
                        print("882 special_case2 == \"within\" and computer_polygon.within(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
                        ...
                        if has_value:
                            ...
                            patch_polygon)):  # within/intersects
                        print("897 special_case2 == \"within\" and computer_polygon.intersects(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
                        ...
                        if polygon_intersect.geom_type == 'MultiPolygon':
                            ...
                            patch_polygon)):  # intersects/within
                        print("932 special_case2 == \"intersects\" and computer_polygon.within(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
                        ...
                        if polygon_intersect.geom_type == 'MultiPolygon':
                            ...
                            patch_polygon)):  # intersects/intersects
                        print("968 special_case2 == \"intersects\" and computer_polygon.intersects(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
            ...
            if (special_case1 == "both" and tumorFlag == "tumor"):
                print("1006 special_case1 == \"both\" and tumorFlag == \"tumor\"")
                if (findPixelWithinPolygon):
                    mask = initial_grid.reshape(patch_size, patch_size)
                    for index1, row in enumerate(mask):
                        for index2, pixel in enumerate(row):
                ...
                # print "\n"
                print "what is this ", title_index
            ...
            if (record_count > 0 and special_case1 == "both"):
                print("1099 record_count > 0 and special_case1 == \"both\"")
                for item in nucleus_item_list:
                    computer_polygon = item[0]
                    Flatness_value = float(item[1])
                    Perimeter_value = float(item[2])
                    ...
                    if (special_case2 == "within" and computer_polygon.within(patch_polygon)):  # within/within
                        print("1125 special_case2 == \"within\" and computer_polygon.within(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
                        ...
                        if has_value:
                            ...
                            patch_polygon)):  # within/intersects
                        print("1142 special_case2 == \"within\" and computer_polygon.intersects(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
                        ...
                        if polygon_intersect.geom_type == 'MultiPolygon':
                            ...
                            patch_polygon)):  # intersects/within
                        print("1177 special_case2 == \"intersects\" and computer_polygon.within(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
                        ...
                        if polygon_intersect.geom_type == 'MultiPolygon':
                            ...
                            patch_polygon)):  # intersects/intersects
                        print("1213 special_case2 == \"intersects\" and computer_polygon.intersects(patch_polygon)")
                        Flatness_list.append(Flatness_value)
                        Perimeter_list.append(Perimeter_value)
                        Circularity_list.append(Circularity_value)
                        r_GradientMean_list.append(r_GradientMean_value)
            ...
            if (special_case1 == "both" and tumorFlag == "non_tumor"):
                print("1252 special_case1 == \"both\" and tumorFlag == \"non_tumor\"")
                if (findPixelWithinPolygon):
                    mask = initial_grid.reshape(patch_size, patch_size)
                    for index1, row in enumerate(mask):
                        for index2, pixel in enumerate(row):
            ...
            for index, tile_min_point in enumerate(unique_tile_min_point_list):
                ...
                for prefix in prefix_list:
                    ...
                    for csv_json_file in os.listdir(detail_local_folder):
                        if csv_json_file.endswith("features.csv") and csv_json_file.find(
                                tmp_string) != -1:  # find it!
                            print('Reading ', csv_json_file, '...')
                ...
                for i in range(0, i_range):
                    for j in range(0, j_range):
                        ...
                        executor.submit(process_one_patch, case_id, user, index, patch_polygon_area, image_width,
                                        image_height, patch_polygon_0, patchHumanMarkupRelation_tumor,
                                        patchHumanMarkupRelation_nontumor,
                                        patch_humanmarkup_intersect_polygon_tumor,
                                        patch_humanmarkup_intersect_polygon_nontumor, nucleus_item_list)
        img.close()
    client.close()
    exit()
